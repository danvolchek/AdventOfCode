...
func calcDistance(curr string, remaining []string, distances map[string]map[string]int) int {
	...
	for i, destination := range remaining {
		min = lib.Min(min, distances[curr][destination]+calcDistance(destination, lib.Remove(remaining, i), distances))
	}
...
func solve(instructions []distance) int {
	...
	for i, location := range locations {
		min = lib.Min(min, calcDistance(location, lib.Remove(locations, i), distances))
	}
