	...
	for _, xOffset := range []int{-1, 0, 1} {
		for _, yOffset := range []int{-1, 0, 1} {
			for _, zOffset := range []int{-1, 0, 1} {
				for _, qOffset := range []int{-1, 0, 1} {
					if xOffset == 0 && yOffset == 0 && zOffset == 0 && qOffset == 0 {
						continue
					}

					pos := coord{val.x + xOffset, val.y + yOffset, val.z + zOffset, val.q + qOffset}
